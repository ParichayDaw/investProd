...
function Stock() {
  ...
  // const debouncedFetch = (symbol, days) => {
  ...
  const fetch = async (symbol, days) => {
    ...
    const options = {
      ...
      headers: {
        ...
        'X-RapidAPI-Key': 'f30a96c477mshf87ea58425b53a6p1ecd86jsn969f5bac77aa',
  ...
  async function handleChange(event) {
    ...
    // debouncedFetch(searchKeyword, 7);
  }
    ...
    >
      ...
      >
        <InputGroup>
          ...
          <Input placeholder='search any stock' textAlign='center' bgColor='#fff'
            fontWeight='semibold'
            isInvalid
    ...
    <Box w={['90vw', '40vw']}>
      ...
      <HStack>
        <Link mr='2' fontSize={['sm', 'md', 'lg']} onClick={() => fetch(company.symbol, 7)}>
          <Tag size={['sm', 'md', 'lg']} fontWeight='bold' variant='solid' colorScheme='whatsapp'>1 week</Tag>
        </Link>
        <Link mr='2' fontSize={['sm', 'md', 'lg']} onClick={() => fetch(company.symbol, 30)}>
          <Tag size={['sm', 'md', 'lg']} fontWeight='bold' variant='solid' colorScheme='whatsapp'>1 month</Tag>
        </Link>
        <Link mr='2' fontSize={['sm', 'md', 'lg']} onClick={() => fetch(company.symbol, 60)}>
          <Tag size={['sm', 'md', 'lg']} fontWeight='bold' variant='solid' colorScheme='whatsapp'>3 month</Tag>
        </Link>
      </HStack>
    </Box>
    <Box position="relative"> {/* Added positioning for the loading animation */}
      <Box className='chart' w={{ base: "80vw", lg: "60vw" }} h='50vh' position='relative'>
        {loading && <Loading />} {/* Rendering loading animation if loading is true */}
        {!loading && <Line data={prices} options={chartOptions} />} {/* Rendering Line chart if loading is false */}
      </Box>
...
//       }));
//       setlist(l);
//       if (l[0][0] < l[days - 1][0]) setGraphColor('#81c995');
//       else setGraphColor('#f93625');
//       setLoading(false);
//     } catch (error) {
//       console.error(error);
//     }
...
//             <Box className='chart' w={{ base: "80vw", lg: "60vw" }} h='50vh' position='relative'>
//               {loading && <Loading />}
//               {!loading && <Line data={prices} options={chartOptions} />}
//             </Box>
//             <Text fontSize={['3xl', '4xl', '5xl']} textAlign='center'>Latest Price: {'\u20B9'}{company.latestPrice}</Text>
//           </Box>
//         </Box>
